...
void ScaleLayer<Dtype>::Forward_gpu(
  ...
  for(int i = 0; i < top[0]->count(); i++){
      if(i == 5530)
          printf("%.10f\n", top[0]->mutable_cpu_data()[i]);
    int ttt = static_cast<int>(round(top[0]->mutable_cpu_data()[i]* 10));
    //top[0]->mutable_cpu_data()[i] = 1.0 * ttt/ 10;  
    //top[0]->mutable_cpu_data()[i] = (2.0 * rand() - rand()) / RAND_MAX; 
  }
}
